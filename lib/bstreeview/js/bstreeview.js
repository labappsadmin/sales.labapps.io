...
; (function ($, window, document, undefined) {
    ...
    $.extend(bstreeView.prototype, {
        ...
        init: function () {
            ...
            // Initialize root node with nodeId
            var rootNode = { nodeId: -1, nodes: this.tree };
            this.nodes.push(rootNode);
            this.initData(rootNode);
            var _this = this;
        ...
        build: function (parentElement, nodes, depth) {
            ...
            $.each(nodes, function addNodes(id, node) {
                // Ensure node has nodeId (should be set by initData)
                if (typeof node.nodeId === 'undefined') {
                    console.warn('Node missing nodeId:', node);
                    return; // Skip this node if no nodeId
                }
